            ...
            <div class="space-y-5">
              ...
              <div>
                <label for="text" class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">
                  Email или телефон<span class="text-error-500">*</span>
                </label>
                <input
              ...
              <div class="flex items-center justify-between">
                <div>
                  <label for="keepLoggedIn" class="flex items-center text-sm font-normal text-gray-700 cursor-pointer select-none dark:text-gray-400">
                    <div class="relative">
                      <input
                        v-model="credentials.keepLoggedIn"
                        type="checkbox"
                        id="keepLoggedIn"
                        class="sr-only"
                      />
                      <div :class="credentials. keepLoggedIn ? 'border-brand-500 bg-brand-500' : 'bg-transparent border-gray-300 dark:border-gray-700'" class="mr-3 flex h-5 w-5 items-center justify-center rounded-md border-[1.25px]">
...
watch(() => credentials.value.login, (newVal) => {
  credentials.value.login = newVal.replace(/\s/g, '');
});

watch(() => credentials.value.password, (newVal) => {
  credentials.value.password = newVal.replace(/\s/g, '');
});
